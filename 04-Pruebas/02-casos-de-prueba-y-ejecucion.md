...
A continuación se documentan **11 casos de prueba** formalmente estructurados, ejecutados sobre el sistema en su entorno local (Node.js + XAMPP + Groq API), utilizando Postman/Thunder Client para las pruebas de API, el navegador web para pruebas E2E, y phpMyAdmin para la verificación de persistencia de datos.
...
## CP-10 — Simulación de Caída de MySQL o Falla de Red con Groq API

| Campo | Detalle |
|---|---|
| **ID** | CP-10 |
| **Categoría** | Manejo de Errores y Casos Límite |
| **Módulo** | `src/config/db.js`, `aiService.js`, manejo global de errores |
| **Precondiciones** | Servidor Node.js en ejecución |
| **Pasos** | 1. Detener manualmente el servicio MySQL desde XAMPP y enviar una solicitud que requiera acceso a la base de datos.<br>2. Restaurar MySQL.<br>3. Simular una falla de red/desconexión y enviar una solicitud de análisis de IA |
| **Datos de Entrada** | Solicitud `GET /api/repos` (con MySQL detenido); solicitud `POST /api/ai/analizar` (con red desconectada) |
| **Resultado Esperado** | El sistema debe capturar el error de forma controlada (sin caída del servidor), responder con un código HTTP apropiado (500 o 503) y registrar el incidente en el log de errores correspondiente |
| **Resultado Obtenido** | El servidor manejó ambos escenarios sin caerse; se retornaron errores controlados (HTTP 500) y se registraron los eventos en consola/`logs_errores` |
| **Estado** | ✅ PASÓ |
...
> El detalle del defecto encontrado durante la ejecución de **CP-05** (falla inicial en la consulta a la IA) se documenta formalmente en el Archivo 3: `03-registro-de-defectos-y-conclusiones.md`.